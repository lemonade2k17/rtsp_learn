...
{
  return new FrameSink(env, subsession, onFrameFunc, ctx);
}
                                  ...
                                  struct timeval presentationTime, unsigned durationInMicroseconds)
{
  ...
  sink->afterGettingFrame0(frameSize, numTruncatedBytes, presentationTime);
}
                                   ...
                                   struct timeval presentationTime)
{
  ...
  if (fOnFrame != nullptr)
  {
    // live555 的展示时间戳是 timeval(秒+微秒), 转成毫秒喂给 OnFrame
    unsigned long long pts_ms = (unsigned long long)presentationTime.tv_sec * 1000
      + presentationTime.tv_usec / 1000;
    fOnFrame(fCtx, fReceiveBuffer, frame_size, pts_ms,
...
void UpstreamSession::setupNextSubsession()
{
  ...
  *fEnv << "  本地接收端口: RTP " << fCurSubsession->clientPortNum()
  ...
  fCurSubsession->sink = FrameSink::CreateNew(*fEnv, *fCurSubsession, fOnFrame, fCtx);
